...
void
myprint(pagetable_t pagetable, int level)
{
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      ...
      print_dot(level);
      printf("%d: pte %p pa %p\n", i, pte, child);
      myprint((pagetable_t)child, level + 1);    
    } else if(pte & PTE_V){
      uint64 pa = PTE2PA(pte);
      print_dot(level);
      printf("%d: pte %p pa %p\n", i, pte, pa);
...
int
ukmove(pagetable_t upagetable, pagetable_t kpagetable, uint64 oldsz, uint64 newsz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = oldsz; i < newsz; i += PGSIZE){
    if((pte = walk(upagetable, i, 0)) == 0)
      panic("ukmove: pte should exist");
    if((*pte & PTE_V) == 0)
      panic("ukmove: page not present");
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    flags = flags & (~PTE_U);
